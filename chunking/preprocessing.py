...
def preprocess_layerwise(
    ...
    #flat_params = {k: jnp.concatenate([v.flatten() for v in layer.values()])
    #        for k, layer in params.items()} 
    #original_lengths = {k: v.size for k, v in flat_params.items()}
    # chunk each layer separately
    #flat_params = {k: chunk_array(v, chunk_size) for k, v in flat_params.items()}
    ...
    flat_params, unflatten = jax.flatten_util.ravel_pytree(flat_params)
